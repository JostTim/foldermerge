...
class StageMixin:
    """This Mixin class is made to be subclassed to allow classes that revolve around a dataframe
    that will be read/saved to disk often, and that must be written to disk at current stage
    if the full code execution fails. For this, it is also a context manager.
    The information relative to the last "write" of the core dataframe to disk, will be
    avaiable in the json file related to the class instance.
    The attributes that must be defined for this behaviour are :
    - name (what will be used to load the dataframe from the RESULTS_PATH folder)
        by default RESULTS_PATH is a folder named FILE_HASHES in our downloads folder.
    ...
    def load(self):
        ...
        if self.save_path.is_file():
            ...
            return
        print(
            f"Could not load {self.save_path}. Making _data an empty DataFrame")
    ...
    def read_all(self):
        with open(self.save_path, "r") as f:
            try:
                ...
                print(
                    f"Could not decode the json results file. All contents have been backed up to {filename}")
                return {}
...
class FolderChecker(StageMixin):
    ...
    def sanitize_search_root(self):
        try:
            self.relative_search_path = self.search_root.relative_to(
                self.repo_path)
        except ValueError:
            raise ValueError(
        ...
        for root, dirs, files in tqdm(self.search_root.walk(), desc="Searching"):
            ...
            for file in files:
                ...
                file_record = {"fullpath": str(
                    file_fullpath), "ctime": ctime, "mtime": mtime}
    ...
    def gather_hashes(self):
        ...
        with hlib:
            self._data["hash"] = self.data.progress_apply(
...
class FolderMerger:
    def __init__(
        ...
        if len(sources_repo) != len(search_paths_repo):
            raise ValueError(
                "sources_repo and relative_roots_repo don't have the same length. Check your arguments")
